...
export default {
  ...
  created () {
    ...
    const endTime = localStorage.getItem('endTime') // 获取浏览器存储的发送验证码时间限制的结束时间
  ...
  methods: {
    ...
    async sendCode () {
      ...
      if (!this.isWaiting) { // 没有处于等待状态
        const res = await axios.post('http://8.134.178.190:5000/login_system/register_verification_code', {
        ...
        localStorage.setItem('endTime', endTime) // 存入浏览器
        //启动计时器
        this.isWaiting = true
        this.startTimer()
      }
    ...
    async register () {
      ...
      const res = await axios.post('http://8.134.178.190:5000/login_system/register', {
        mail_account: this.mail_account,
